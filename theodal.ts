...
const countrytheobtn: HTMLElement | null = document.getElementById("countrytheo"); // Knappen för att visa countrytheos musik
const countrymusik: HTMLElement | null = document.getElementById("countrymusik"); // Containern för musiken som vi togglar synligheten på
const rocktheobtn: HTMLElement | null = document.getElementById("rocktheo"); // Knappen för att visa rocktheos musik
const rockmusik: HTMLElement | null = document.getElementById("rockmusik"); // Containern för musiken som vi togglar synligheten på
const poptheobtn: HTMLElement | null = document.getElementById("poptheo"); // Knappen för att visa poptheos musik
const popmusik: HTMLElement | null = document.getElementById("popmusik"); // Containern för musiken som vi togglar synligheten på
...
const ololtheobtn: HTMLElement | null = document.getElementById("ololtheo"); // Knappen för att visa ittheos musik
const ololmusik: HTMLElement | null = document.getElementById("ololmusik"); // Containern för musiken som vi togglar synlig
...
function shuffle_queue(): void {
    ...
    for (let i = 0; i < queuearray.length; i++) {       // Ser till att kön matchar
...
function show_lyrics_for(songId: string) {
    if (!box) return;
...
co_firm ? co_firm.addEventListener("click", () => {
    ...
    const currentsongname = playing ? playing?.textContent : null;
    const quecopy = [...queuearray];
...
repQBTN ? repQBTN.addEventListener("click", () => {
    if (repqueue) {
        repqueue = false;
...
skipbtn ? skipbtn.addEventListener("click", () => { skip(playing ? playing.textContent : "error") }) : undefined;
...
if (ittheobtn !== null && itmusik !== null) {
    ittheobtn.addEventListener("click", () => { toggle_hide(itmusik) });
    toggle_hide(itmusik);
}

if (ololtheobtn !== null && ololmusik !== null) {
    ololtheobtn.addEventListener("click", () => { toggle_hide(ololmusik) });
